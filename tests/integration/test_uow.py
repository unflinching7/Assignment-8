...
def insert_slot(session, slot_ref, service_type, slot_version=1):
    session.execute(
        "INSERT INTO service_offerings (service_type, location_number) VALUES (:service_type, :location)",
        dict(service_type=service_type, location=slot_version),
    )
    session.execute(
...
def test_concurrent_updates_to_location_number_are_not_allowed(postgres_session_factory):
    service_type, slot = random_service_type(), random_slot_reference()
    session = postgres_session_factory()
    insert_slot(session, slot, service_type,
                slot_version=1)
    session.commit()

    slots = [random_slot_reference() for _ in range(10)]
